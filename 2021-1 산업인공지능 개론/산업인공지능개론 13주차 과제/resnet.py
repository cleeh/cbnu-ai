...
# , but found libiomp5md.dll already initialized 해결
import os
...
def imshow(inp, title=None):
  ...
  mean = np.array(const_mean)
  std = np.array(const_std)
  inp = std * inp + mean
  inp = np.clip(inp, 0, 1)
  plt.imshow(inp)
...
if __name__ == '__main__':
  ...
  num_ftrs = model_ft.fc.in_features # 모델에서 feature extraction 후 FC 층에 입력되는 특징 수
